...
import { loadMonetagZoneScript, showMonetagRewardedAd } from "@/lib/monetagSdk";
...
export default function TaskPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/settings?initData=${encodeURIComponent(initData)}`)
      .then((r) => r.json())
      .then(async (d) => {
        const zoneId = d.settings?.monetagZoneId;
  ...
  async function startTask(task: Task) {
    ...
    try {
      ...
      if (data.adNetwork === "monetag" && monetagZoneId) {
        try {
          // preload -> show chain exactly as Monetag docs recommend, tied by ymid=requestId
          await showMonetagRewardedAd(monetagZoneId, data.requestId);
